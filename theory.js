...
(function () {
     ...
     *  • квинта D7 (II) → I вниз; прима (V) в басу D7 → I вверх;
     *  • общий тон (V) в обращениях остаётся на месте.
     * Результат удвоений (как в учебнике):
     *  • D7 → t3   — неполная тоника: утроенная прима на одной высоте + терция (без квинты);
     *  • D6/5 → t53 — полная тоника, удвоенная прима (унисон);
     *  • D4/3 → t53 — полная тоника, прима снизу и сверху (октава);
     *  • D2 → t6   — тонический секстаккорд (терция в басу), удвоенная прима (унисон).
     */
    function getD7Voicings(tonic, mode) {
        // Регистр: тоники C/C#/D/D#/E ставим в окт.4 (иначе уходят слишком низко),
        // F..B — в окт.3, чтобы аккорды сидели по центру нотного стана в любой тональности.
        const tpc = pc({ ...tonic, octave: 0 });
        const baseOct = (tpc <= 4) ? 4 : 3;
        const anchor = { ...tonic, octave: baseOct };
        const S3 = mode === 'major' ? 4 : 3;      // полутоны до III ступени
        const v = (deg, semis) => buildIntervalUp(anchor, deg, semis);

        // ЭТАЛОН solfeggio-online.ru: D7 и обращения — ТЕСНОЕ расположение (звуки по терциям),
        // обращение = перенос нижнего звука на октаву вверх. Ступени: 1=I(база) … 8=I(окт+1) …
        // Тоны D7: V=прима, VII#(=11 п/т)=терция, II=квинта, IV=септима.
        // Разрешения (по правилам страницы-эталона / учебника):
        //   D7 → T3   : утроенная прима НА ОДНОЙ высоте (три головки: две рядом + ещё одна)
        //               + терция; БЕЗ квинты, БЕЗ октавы вверх/вниз;
        //   D6/5 → T53: удвоенная прима (тесно, унисон);
        //   D4/3 → T53: развёрнутое, прима снизу И сверху (через октаву);
        //   D2 → T6   : секстаккорд, удвоенная прима (унисон).
        // ВАЖНО: повторяющиеся (унисонные) ступени в разрешениях НЕ схлопываем —
        // они рисуются двумя головками рядом, как в учебнике (утроенная/удвоенная прима).
        const forms = [
            {
                label: 'D7', target: 'T3',
                chord: [[5, 7], [7, 11], [9, 14], [11, 17]],                 // V  VII#  II  IV  (тесно)
                // Голосоведение: V→I↑, VII#→I↑, II→I↓, IV→III↓ → три I на одной высоте + III
                res:   [[8, 12], [8, 12], [8, 12], [10, 12 + S3]]           // I I I(тройной унисон) III — утроенная прима без квинты
            },
            {
                label: 'D6/5', target: 'T53',
                chord: [[7, 11], [9, 14], [11, 17], [12, 19]],               // VII# II  IV  V
                res:   [[8, 12], [8, 12], [10, 12 + S3], [12, 19]]          // I I(унисон) III V — удвоенная прима
            },
            {
                label: 'D4/3', target: 'T53',
                chord: [[9, 14], [11, 17], [12, 19], [14, 23]],              // II  IV  V  VII#
                res:   [[8, 12], [10, 12 + S3], [12, 19], [15, 24]]         // I(низ) III V I(верх) — развёрнутое, прима снизу и сверху
            },
            {
                label: 'D2', target: 'T6',
                chord: [[11, 17], [12, 19], [14, 23], [16, 26]],             // IV  V  VII#  II
                res:   [[10, 12 + S3], [12, 19], [15, 24], [15, 24]]        // III(бас) V I I(унисон) — секстаккорд, удвоенная прима
            }
        ];
